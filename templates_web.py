# Logo SVG moderna e minimalista
...
# Modal de Novo Registro (Pop-up com fundo desfocado)
ADD_MODAL = """
<div id="addModal" class="fixed inset-0 z-50 hidden" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div class="fixed inset-0 bg-black/60 backdrop-blur-sm transition-opacity opacity-0 modal-backdrop" onclick="toggleModal(false)"></div>

    <div class="fixed inset-0 z-10 overflow-y-auto">
        <div class="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
            <div class="relative transform overflow-hidden rounded-2xl bg-[#111827] border border-slate-800 text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-lg opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95 modal-panel">
                
                <div class="bg-[#1F2937]/50 px-4 py-3 sm:px-6 border-b border-slate-800 flex justify-between items-center">
                    <h3 class="text-base font-semibold leading-6 text-white" id="modal-title">Novo Registro</h3>
                    <button type="button" onclick="toggleModal(false)" class="text-slate-400 hover:text-white transition">✕</button>
                </div>
                ...
                <form action="/transaction/new" method="POST" class="p-6 space-y-5">
                    <div class="grid grid-cols-2 gap-2 p-1 bg-slate-900 rounded-lg border border-slate-800">
                        <label class="cursor-pointer">
                            ...
                            <div class="text-center py-2 rounded-md text-slate-500 text-xs font-bold uppercase tracking-wider peer-checked:bg-red-500/10 peer-checked:text-red-500 transition">Saída</div>
                        </label>
                        <label class="cursor-pointer">
                            <input type="radio" name="type" value="income" class="peer sr-only">
                            <div class="text-center py-2 rounded-md text-slate-500 text-xs font-bold uppercase tracking-wider peer-checked:bg-green-500/10 peer-checked:text-green-500 transition">Entrada</div>
                        </label>
                    </div>

                    <div class="space-y-4">
                        <div>
                            <label class="block text-xs font-medium text-slate-400 uppercase mb-1">Descrição</label>
                            <input type="text" name="description" required class="w-full input-dark rounded-lg px-4 py-2.5 text-sm" placeholder="Ex: Supermercado">
                        </div>
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-xs font-medium text-slate-400 uppercase mb-1">Valor (R$)</label>
                                <input type="number" step="0.01" name="amount" required class="w-full input-dark rounded-lg px-4 py-2.5 text-sm font-mono" placeholder="0.00">
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-slate-400 uppercase mb-1">Categoria</label>
                                <input type="text" name="category" class="w-full input-dark rounded-lg px-4 py-2.5 text-sm" placeholder="Geral">
                            </div>
                        </div>
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-xs font-medium text-slate-400 uppercase mb-1">Método</label>
                                <select name="payment_method" class="w-full input-dark rounded-lg px-3 py-2.5 text-sm bg-[#111827]">
                            ...
                            <div>
                                <label class="block text-xs font-medium text-slate-400 uppercase mb-1">Conta</label>
                                <select name="account_id" class="w-full input-dark rounded-lg px-3 py-2.5 text-sm bg-[#111827]">
                                    <option value="">-- Selecionar --</option>
                                    {% for acc in accs %} <option value="{{ acc.id }}">{{ acc.name }}</option>
                                    {% endfor %}
                                    {% for acc in accounts %} <option value="{{ acc.id }}">{{ acc.name }}</option>
                                    {% endfor %}
                                </select>
                            </div>
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-slate-400 uppercase mb-1">Cartão (Se Crédito)</label>
                            <select name="card_id" class="w-full input-dark rounded-lg px-3 py-2.5 text-sm bg-[#111827]">
                        ...
                        <div>
                            <label class="block text-xs font-medium text-slate-400 uppercase mb-1">Data</label>
                            <input type="datetime-local" name="date" value="{{ now }}" class="w-full input-dark rounded-lg px-4 py-2.5 text-sm">
                        </div>
                    </div>

                    <div class="pt-2">
                        <button type="submit" class="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-3 rounded-lg shadow-lg shadow-blue-900/20 transition transform active:scale-[0.98] text-sm">Salvar Registro</button>
                    </div>
                </form>
            </div>
        </div>
...
<body class="antialiased min-h-screen flex flex-col bg-[#0B0F19]">
    ...
    {% if messages %}
        <div id="toast-container" class="fixed bottom-6 right-6 z-[60] flex flex-col gap-2 max-w-[90vw]">
    ...
    <div class="w-full max-w-sm">
        ...
        <div class="glass p-8 rounded-xl shadow-xl border border-slate-800">
            <form method="POST" class="space-y-5">
                <div class="space-y-1.5"><label class="text-xs font-medium text-slate-400 uppercase tracking-wide">Telegram ID</label><input type="text" name="telegram_id" class="w-full input-dark rounded-lg px-4 py-3 text-sm focus:ring-blue-500/20 placeholder-slate-600" placeholder="Seu ID"></div>
                <div class="space-y-1.5"><label class="text-xs font-medium text-slate-400 uppercase tracking-wide">Senha</label><input type="password" name="password" class="w-full input-dark rounded-lg px-4 py-3 text-sm focus:ring-blue-500/20 placeholder-slate-600" placeholder="••••••••"></div>
                <button type="submit" class="w-full bg-blue-600 hover:bg-blue-500 text-white font-medium py-3 rounded-lg transition-all shadow-lg shadow-blue-900/20 text-sm mt-2">Acessar</button>
            </form>
        </div>
        <div class="mt-8 text-center"><a href="/register" class="text-sm text-slate-500 hover:text-blue-400 transition">Primeiro acesso?</a></div>
    </div>
</div>
"""
...
<div class="flex flex-col items-center justify-center min-h-screen px-4 bg-[#0B0F19]">
    <div class="w-full max-w-sm glass p-8 rounded-xl border border-slate-800 relative">
        <button onclick="history.back()" class="absolute top-6 left-6 text-slate-500 hover:text-white transition text-sm flex items-center gap-1">← Voltar</button>
...
<div class="flex flex-col items-center justify-center min-h-screen px-4 bg-[#0B0F19]">
    <div class="w-full max-w-sm glass p-8 rounded-xl border border-slate-800">
        ...
        <form method="POST" action="/verify_setup" class="space-y-5">
            ...
            <div><label class="text-xs font-medium text-slate-400 uppercase tracking-wide">Nova Senha</label><input type="password" name="password" class="w-full mt-1 input-dark rounded-lg px-4 py-3 text-sm" required></div>
...
<nav class="sticky top-0 z-40 bg-[#0B0F19]/80 backdrop-blur-md border-b border-slate-800">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-16">
            <div class="flex items-center gap-3">{LOGO_SVG}<span class="font-semibold text-white tracking-tight text-lg">Zenith</span></div>
...
<main class="max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div onclick="toggleModal(true)" class="glass p-5 rounded-xl border border-dashed border-slate-700 hover:bg-blue-500/5 transition flex items-center justify-center gap-3 group cursor-pointer h-full">
